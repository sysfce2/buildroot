...
################################################################################
# batocera - bump
PYTHON_PYQT5_VERSION = 5.15.11
PYTHON_PYQT5_SOURCE = PyQt5-$(PYTHON_PYQT5_VERSION).tar.gz
PYTHON_PYQT5_SITE = https://files.pythonhosted.org/packages/0e/07/c9ed0bd428df6f87183fca565a79fee19fa7c88c7f00a7f011ab4379e77a
...
PYTHON_PYQT5_DEPENDENCIES += host-pyqt-builder # batocera
...
define PYTHON_PYQT5_CONFIGURE_CMDS
	...
	(cd $(@D); \
		...
		$(HOST_DIR)/bin/sip-build \
			$(PYTHON_PYQT5_CONF_OPTS) \
	)
endef

define PYTHON_PYQT5_BUILD_CMDS
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE) -C $(@D)/build
endef
...
define PYTHON_PYQT5_INSTALL_TARGET_CMDS
	...
	$(TARGET_MAKE_ENV) $(TARGET_CONFIGURE_OPTS) $(MAKE1) -C $(@D)/build install
